...
module.exports = class PFCCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'pfc',
            group: 'texte',
            memberName: 'pfc',
            description: "Aronte qui tu veux dans un duel à mort",
        });
    }
    ...
    async run(msg, { args }) {
        ...
        if(args == null){
          ...
          msg.say(`${pfc1[Math.floor(Math.random() * pfc1.length)]}`)
          const filter_1_1 = m => m.author.id === Client.user.id && m.content === `On rappelle pour les débiles :  pierre :fist:    feuille :raised_hand:    ciseaux :v:`;
          const collector_1_1 = msg.channel.createMessageCollector(filter_1_1, { max: 1,time: temps });
          var symb = pfc[Math.floor(Math.random() * pfc.length)]
          ...
          if(!joueur_2) {//Si l'id est correct
            ...
            return msg.channel.send(`**${args}**, Putain mais c'est personne`)
          }
          ...
          joueur_1.createDM().then(channel_1 => {
            ...
            //---------------------------Message au joueur 1
            const filter_2_0 = m => m.author.id === Client.user.id && m.content === `On rappelle pour les débiles :  pierre :fist:    feuille :raised_hand:    ciseaux :v:`;
          ...
          //---------------------------Message au joueur 2
                joueur_2.createDM().then(channel_2 => {
                  ...
                  const filter_2_01 = m => m.author.id === Client.user.id && m.content === `On rappelle pour les débiles :  pierre :fist:    feuille :raised_hand:    ciseaux :v:`;
